...
def advance_chess768_accumulators_into(
    ...
    Chess768 has no king-bucket frame, so even king moves are sparse deltas.  Promotions,
    en-passant and castling are handled explicitly from the same encoded move semantics as V13.
    """
...
def _div_trunc_zero(value: int, divisor: int) -> int:
    # Rust signed integer division truncates toward zero; Python's // floors negatives.
    if value >= 0:
...
def evaluate_chess768_accumulators(
    ...
    for neuron in range(hidden):
        output += _screlu_quantised(int(them[neuron])) * int(
            output_weights[hidden + neuron]
        )
...
def load_bullet_quantised(path: str | Path, hidden_size: int) -> tuple[np.ndarray, ...]:
    ...
    SavedFormat is `l0w, l0b, l1w, l1b`, all little-endian i16.  Bullet writes l0w in its native
    column-major `H x 768` layout, which means each input feature's H weights are contiguous on disk;
    reshaping the byte stream as `(768, H)` is therefore exactly the layout incremental Numba wants.
    Trailing bytes are only Bullet's documented 64-byte file padding.
    """
