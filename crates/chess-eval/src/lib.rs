...
//!
//! M5 experiments may opt into a frozen learned residual through the
//! `CHESS_EXPERIMENTAL_RESIDUAL_NNUE` environment variable. The default path remains exactly the
//! accepted classical evaluator. The experimental model is loaded once per process and adds the
//! validation-selected `33/16` residual correction. This deliberately simple full-refresh path is
//! only for the first causal playing-strength screen; accepted learned evaluation will use the
//! already-proven incremental accumulator.
...
const EXPERIMENTAL_RESIDUAL_ENV: &str = "CHESS_EXPERIMENTAL_RESIDUAL_NNUE";
const RESIDUAL_SCALE_NUMERATOR: i64 = 33;
const RESIDUAL_SCALE_DENOMINATOR: i64 = 16;
static EXPERIMENTAL_RESIDUAL: OnceLock<Option<Network>> = OnceLock::new();
...
///
/// Production/default processes return the accepted classical score exactly. A process explicitly
/// configured with `CHESS_EXPERIMENTAL_RESIDUAL_NNUE=/path/to/network.nnue` adds the frozen M5
/// residual correction. Loading is lazy and happens once; malformed or missing configured models
/// fail loudly rather than silently changing the experiment back to the control.
#[must_use]
pub fn evaluate(position: &Position) -> i32 {
    let classical = evaluate_classical(position);
    let Some(network) = experimental_residual_network() else {
        return classical;
    };
    let residual = network
        .evaluate_full(position)
        .expect("legal search positions contain both kings");
    let correction = i64::from(residual) * RESIDUAL_SCALE_NUMERATOR / RESIDUAL_SCALE_DENOMINATOR;
    i64::from(classical)
        .saturating_add(correction)
        .clamp(i64::from(i32::MIN), i64::from(i32::MAX)) as i32
}

/// Accepted handcrafted evaluator without any M5 experimental overlay.
///
/// Offline teacher tooling uses this entry point so classical targets stay stable even when the
/// surrounding process happens to carry an experimental NNUE environment variable.
#[must_use]
...
fn experimental_residual_network() -> Option<&'static Network> {
    EXPERIMENTAL_RESIDUAL
        .get_or_init(|| {
            let Ok(path) = env::var(EXPERIMENTAL_RESIDUAL_ENV) else {
                return None;
            };
            if path.is_empty() {
                panic!("{EXPERIMENTAL_RESIDUAL_ENV} must not be empty when configured");
            }
            let bytes = fs::read(&path).unwrap_or_else(|error| {
                panic!("failed to read experimental residual NNUE {path}: {error}")
            });
            Some(Network::from_bytes(&bytes).unwrap_or_else(|error| {
                panic!("failed to load experimental residual NNUE {path}: {error}")
            }))
        })
        .as_ref()
}
