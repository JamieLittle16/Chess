...
impl MoveList {
    ...
    /// Borrow the populated prefix as mutable scratch storage.
    ///
    /// Search may reorder a freshly generated move list in place. The list owns its fixed backing
    /// array, so doing so neither allocates nor mutates chess position state.
    #[must_use]
