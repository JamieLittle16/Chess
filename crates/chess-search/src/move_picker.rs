...
///
/// The picker deliberately mutates only the order of the freshly generated `MoveList`. It does not
/// allocate, clone the board, or pre-score/sort moves that a beta cutoff may make irrelevant.
/// Tactical moves are selected lazily by a cheap MVV/LVA-plus-promotion score. Later M4 stages can
/// replace that score with SEE and add killer/history/counter-move stages without changing search's
/// ownership model.
...
impl<'a> MovePicker<'a> {
    ...
    /// Select the next move, scoring only the current stage on demand.
...
mod tests {
    ...
    #[test]
    fn tt_move_is_returned_first_without_full_sorting() {
