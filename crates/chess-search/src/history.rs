...
impl HistoryTables {
    ...
    ///
    /// Positive bonus rewards a quiet that causes a scout-node cutoff; negative bonus penalises a
    /// quiet that was actually searched and failed to cut. The gravity term naturally damps old
    /// evidence instead of saturating every frequently visited move at one extreme.
    pub(super) fn update(
        &mut self,
        side: Color,
        previous: Option<MoveContext>,
