...
def load_openings(path: Path, offset: int, pairs: int) -> list[str]:
    openings = [line.strip() for line in path.read_text().splitlines() if line.strip() and not line.startswith("#")]
    selected = openings[offset : offset + pairs]
    if len(selected) != pairs:
        raise SystemExit(f"requested {pairs} openings from offset {offset}, only {len(selected)} available")
    for fen in selected:
        chess.Board(fen)
    return selected
...
def play_game(candidate: Worker, control: Worker, fen: str, candidate_white: bool, nodes: int, max_plies: int) -> tuple[float, str, int]:
    ...
    for _ in range(max_plies):
        ...
        if outcome is not None:
            ...
            return (1.0 if outcome.winner == candidate_white else 0.0, board.result(claim_draw=True), board.ply())
    ...
    try:
        ...
        for opening_index, fen in enumerate(openings, start=args.opening_offset):
            for candidate_white in (True, False):
                ...
                row = {"opening": opening_index, "candidate_white": candidate_white, "candidate_score": score, "result": result, "plies": plies}
                rows.append(row)
...
def main() -> int:
    args = parse_args()
    return worker_main(args.worker.resolve()) if args.worker is not None else controller_main(args)
