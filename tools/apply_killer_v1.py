...
picker = picker.replace(
    "MovePicker::new(&mut moves, Some(tt_move))",
    "MovePicker::new(&mut moves, Some(tt_move), [None; 2])",
)
picker = picker.replace(
...
picker = picker.replace(
    "MovePicker::new(&mut scratch, Some(original.as_slice()[3]))",
    "MovePicker::new(&mut scratch, Some(original.as_slice()[3]), [None; 2])",
)
picker = replace_once(
    picker,
