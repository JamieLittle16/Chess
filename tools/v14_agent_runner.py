...
import numpy as np
protocol=os.fdopen(os.dup(1),'w');os.dup2(2,1);sys.path.insert(0,sys.argv[1]);agent=import_module('agent')
def reset():
...
for line in sys.stdin:
    q=json.loads(line)
    if q.get('reset') is True:reset();protocol.write('{"reset":true}\n');protocol.flush();continue
