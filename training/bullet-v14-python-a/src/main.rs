...
const ALLOWED_HIDDEN_SIZES: [usize; 4] = [64, 96, 128, 192];
...
fn usage(reason: &str) -> String {
    format!(
        "{reason}\nusage: chess-bullet-v14-python-a <viri|sf|bullet> <data> <output-dir> <64|96|128|192> [superbatches] [batches-per-superbatch]"
    )
}

fn parse_hidden_size(value: String) -> Result<usize, String> {
    let parsed = value
        .parse::<usize>()
        .map_err(|error| format!("invalid hidden size {value:?}: {error}"))?;
    if !ALLOWED_HIDDEN_SIZES.contains(&parsed) {
        return Err(format!(
            "hidden size {parsed} is outside the frozen V14-A ladder {:?}",
...
fn run() -> Result<(), String> {
    ...
    println!("hidden_size={}", args.hidden_size);
    ...
    let schedule = TrainingSchedule {
        net_id: format!("chess-v14-python-768x{hidden_size}"),
