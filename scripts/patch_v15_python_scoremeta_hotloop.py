...
# Add a single-pass scorer that returns the original score plus two low metadata bits.
...
# Keep original scorer for any dormant helper/reference, and insert metadata scorer after it.
s=s[:end]+'\n\n'+meta_score+s[end+2:]
...
# qsearch: both checked-evasion and tactical-only branches are ordered into score_stack[ply].
...
# negamax: reuse both tactical and pawn bits.
...
# Root is ordered too; only halfmove reset metadata is needed there.
...
# Exactly one occurrence inside _root after insertion; other fallback entry points keep conservative helper.
