...
def one(old,new):
    global s
    n=s.count(old)
    if n!=1: raise SystemExit(f'anchor count {n}: {old[:160]!r}')
...
def _see_find_legal_attacker(
    ...
    # Pawns are the least valuable attackers.
    ...
    # Bishops, rooks, then queens in material-value order.
    ...
    # Kings are last and legality checked against the resulting occupancy.
...
one('''    checked = _state_in_check(board, side, eval_stack[ply])
    if not checked:
''','''    checked = _state_in_check(board, side, eval_stack[ply])
    best_score = -INFINITY
    if not checked:
''')
one('''        if stand_pat >= beta:
...
''','''    for index in range(count):
        ...
        captured_bound = int(PIECE_VALUE[PAWN]) if (move & FLAG_EP) else int(PIECE_VALUE[abs(int(board[target]))])
        delta_candidate = (
            not checked
            and capture
            and not promotion
...
one('''        child_castling, child_ep, captured_piece, captured_square = make_move_inplace(
