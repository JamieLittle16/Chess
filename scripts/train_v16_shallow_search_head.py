...
clipped difference between Rust's 2k-node search value and its own Gestalt static value. We solve for
a weight *delta* around the exact existing head, avoiding any intercept/calibration mismatch.
"""
...
def metrics(base: np.ndarray, pred: np.ndarray, desired: np.ndarray, idx: np.ndarray) -> dict[str, float]:
    if len(idx) == 0:
        return {"rmse_to_desired": 0.0, "mae_to_desired": 0.0, "mean_abs_shift_cp": 0.0,
...
def main() -> int:
    ...
    sample_weight[kinds != "qstable"] = 1.25

    # Linearised score change per one output-weight step. We fit only the correction delta; all
    # fixed biases and quantisation offsets remain embodied in the exact baseline score.
    ...
    ridge_scales = (0.01, 0.03, 0.1, 0.3, 1.0, 3.0, 10.0)
    summary: dict[str, object] = {
        "schema": "v16-shallow-rust-search-h64-head-v2",
        "records": n,
        ...
        "raw_search_minus_static": {
            "mean_cp": float(np.mean(raw_delta)), "median_cp": float(np.median(raw_delta)),
            "mae_cp": float(np.mean(np.abs(raw_delta))), "p95_abs_cp": float(np.quantile(np.abs(raw_delta), 0.95)),
        },
        "presets": [],
    }
    ...
    for spec in presets:
        ...
        desired_delta = fraction * clipped_delta
        desired = base.astype(np.float64) + desired_delta
        yw = desired_delta[tr] * sw
        rhs = Xw.T @ yw
        candidates = []
        for rs in ridge_scales:
            lam = max(1e-9, rs * diag)
            fitted_dw = np.linalg.solve(gram + lam * eye, rhs)
            qdw = np.clip(np.rint(fitted_dw), -max_step, max_step).astype(np.int16)
            qw = (w0.astype(np.int32) + qdw.astype(np.int32)).astype(np.int16)
        ...
        _, rs, qw, vm, hm = min(candidates, key=lambda x: x[0])
        pred = v13 + exact_student(x2, qw, bias, sign)
        np.savez_compressed(a.output_dir / f"shallow-search-head-{spec['name']}.npz",
                            feature_weights=fw, feature_bias=fb, output_weights=qw,
                            output_bias=np.asarray([bias], np.int32))
        delta_w = qw.astype(np.int32) - w0.astype(np.int32)
        entry = {
            **spec, "ridge_scale": rs, "validation": vm, "holdout": hm,
