#!/usr/bin/env python3
"""Train a compact absolute768 V14 student against either V13 or its classical core.

Generation-2 differs from student A in two important ways: substantially larger teacher corpora are
expected, and the baseline is explicit.  `--baseline v13` learns an additive correction on top of
qualified V13.  `--baseline classical` learns the complete correction from V13's inexpensive
classical state to the Stockfish teacher, allowing the old king-piece residual to be removed at
runtime.  Model selection uses validation RMSE only; holdout remains untouched until the best epoch
has been restored.  Float and exact integer inference are both reported.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--hidden", type=int, choices=ALLOWED_HIDDEN, required=True)
    p.add_argument("--baseline", choices=("v13", "classical"), required=True)
...
def load_api(engine_dir: Path) -> tuple[Any, Any, Any, Any, int]:
    ...
    from experiments.numba_search import (
        ...
        _evaluate_state,
        _evaluate_state_classical,
    )
    return encode_position, _build_eval_state_into, _evaluate_state, _evaluate_state_classical, int(EVAL_WIDTH)


def build_dataset(path: Path, engine_dir: Path, baseline_mode: str) -> dict[str, np.ndarray]:
    encode_position, build_state, evaluate_state, evaluate_classical, eval_width = load_api(engine_dir)
    ...
    with path.open(encoding="utf-8") as handle:
        for line in handle:
            ...
            if record.get("teacher_mate") is not None:
                continue
            if abs(int(record["teacher_cp"])) > 5000:
                continue
            rows.append(record)
    if not rows:
    ...
    for i, record in enumerate(rows):
        ...
        base_cp = int(evaluate_state(encoded.side, state)) if baseline_mode == "v13" else int(evaluate_classical(encoded.side, state))
        sf_cp = int(record["teacher_cp"])
        sign = 1.0 if board.turn == chess.WHITE else -1.0
        residual_stm = max(-TARGET_CLIP_CP, min(TARGET_CLIP_CP, sf_cp - base_cp))
        teacher[i] = sf_cp
        baseline[i] = base_cp
        side_sign[i] = sign
    ...
    return {"x": x, "teacher": teacher, "baseline": baseline, "target_abs": target_abs, "side_sign": side_sign, "split_code": split_code}
...
def split_metrics(data: dict[str, np.ndarray], mask: np.ndarray, f: np.ndarray, q: np.ndarray) -> dict[str, float | int]:
    teacher, baseline, sign = data["teacher"][mask], data["baseline"][mask], data["side_sign"][mask]
    fs = baseline + sign * f[mask] * CP_SCALE
    qs = baseline + sign * q[mask]
    return {
        "records": int(mask.sum()),
        "baseline_rmse_cp": rmse(teacher, baseline),
        "float_corrected_rmse_cp": rmse(teacher, fs),
        "quant_corrected_rmse_cp": rmse(teacher, qs),
        "baseline_mae_cp": mae(teacher, baseline),
        "float_corrected_mae_cp": mae(teacher, fs),
...
def main() -> int:
    ...
    data = build_dataset(args.teacher, args.engine_dir, args.baseline)
    ...
    rng = np.random.default_rng(args.seed + args.hidden + (1000 if args.baseline == "classical" else 0))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        f = float_forward(x, w0, b0, w1, b1)
        vm = codes == 1
        score = data["baseline"][vm] + data["side_sign"][vm] * f[vm] * CP_SCALE
        val = rmse(data["teacher"][vm], score)
        row = {"epoch": epoch, "mean_huber": total_loss / max(1, examples), "validation_float_rmse_cp": val}
        history.append(row)
        print(json.dumps(row, sort_keys=True), flush=True)
        if val < best_val:
            best_val, best_epoch, best = val, epoch, [a.copy() for a in arrays]

    assert best is not None
    w0[:], b0[:], w1[:], b1[:] = best
    f = float_forward(x, w0, b0, w1, b1)
    q_w0, q_b0, q_w1, q_b1 = quantize(w0, b0, w1, b1)
    q = quant_forward_cp(x, q_w0, q_b0, q_w1, q_b1)
    metrics = {name: split_metrics(data, codes == code, f, q) for name, code in (("train", 0), ("validation", 1), ("holdout", 2))}
    metadata = {
        "schema_version": 2,
        "architecture": f"absolute768-single-{args.hidden}-screlu-{args.baseline}-baseline",
    ...
    np.savez_compressed(args.output_dir / f"student-h{args.hidden}.npz", feature_weights=q_w0, feature_bias=q_b0, output_weights=q_w1, output_bias=q_b1)
