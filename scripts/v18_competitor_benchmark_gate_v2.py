#!/usr/bin/env python3
"""Aggregate V2 per-engine Chessathon move-benchmark summaries and enforce outperformance."""
...
def load(path: Path) -> dict[str, Any]:
    ...
    if payload.get("schema_version") != 2:
        raise ValueError(f"expected V2 benchmark summary in {path}")
    return payload
...
def main() -> int:
    ...
    outperformance_pass = not missing and not unexpected
    ...
    for target in expected:
        ...
        target_outperformance = all(checks.values())
        regression_pass = regression_pass and target_regression
        outperformance_pass = outperformance_pass and target_outperformance
        ...
        per_target[target] = {
            ...
            "outperformance_pass": target_outperformance,
            "checks": checks,
            "summary_path": source_by_target[target],
        }

    if total_positions:
        aggregate_candidate = weighted_candidate_loss / total_positions
        aggregate_historical = weighted_historical_loss / total_positions
        aggregate_pair = weighted_pair_points / total_positions
    else:
        aggregate_candidate = aggregate_historical = aggregate_pair = float("nan")
    ...
    result = {
        ...
        "outperformance_gate_pass": outperformance_pass,
    ...
    print(encoded, end="")
    return 0 if outperformance_pass else 1
