#!/usr/bin/env python3
"""Apply ROOT-VALUE4 only above a chosen material-phase threshold."""
...
def main() -> int:
    ...
    ap.add_argument("--min-phase", type=int, required=True)
    args = ap.parse_args()
    if not 0 <= args.min_phase <= 24:
        raise SystemExit("--min-phase must be in [0,24]")
