...
def _qsearch_tt_store(
    ...
    # Keep qsearch low priority: never evict current-generation full-search work.
    replace = (
...
one(
    '''    if tt_match and _tt_meta_depth(meta) >= depth:
        tt_score = _tt_meta_score(meta, ply)
''',
...
if s.count('TT_QDEPTH_BASE') < 5:
    raise SystemExit('qdepth marker not wired')
if s.count('history_contexts,\n            tt_table,') < 2:
    raise SystemExit('qsearch state not wired')
p.write_text(s)
