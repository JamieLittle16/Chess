...
Root children remain eagerly materialized. Every interior negamax edge carries one packed uint64
containing the move, signed moving piece and signed captured piece. A zero edge means the H64 row is
valid; the high bit means it is deferred. At depth zero we walk back to the nearest valid ancestor
and replay the exact absolute768 accumulator deltas forward once. Thus TT/futility/cutoff branches
pay no H64 transport, while any edge that actually feeds qsearch is materialized at most once for the
current DFS position. Search/evaluation values are intended to remain bit-identical.
"""
...
# First stateful root call is expanded one argument per line.
old='''            hash_moves,\n            history_contexts,\n            tt_table,\n'''
new='''            hash_moves,\n            history_contexts,\n            student_edges,\n            tt_table,\n'''
if s.count(old)!=1: raise SystemExit(f'expanded root tail count={s.count(old)}')
s=s.replace(old,new,1)
# Adaptive/timed calls are compact near the tail.
