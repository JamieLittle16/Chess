#!/usr/bin/env python3
"""Add a score-disabled compact Chess768 accumulator of a selected width to exact V13.

Used only for whole-search deployment-cost measurement. The synthetic network score is never read,
so fixed-node search signatures must remain identical to V13 for every width.
"""
...
ALLOWED = (64, 96, 128, 192)


def patch(path: Path, hidden: int) -> None:
    if hidden not in ALLOWED:
        raise SystemExit(f"hidden width must be one of {ALLOWED}")
    source = path.read_text()
    if "NNUE_COST_HIDDEN" in source:
        raise SystemExit("source already contains a search-cost probe")
    ...
    block = f'''NNUE_COST_HIDDEN = {hidden}
...
'''
    source = source.replace(width_marker, block, 1)
    ...
    if source.count(build_marker) != 1 or source.count(advance_marker) != 1:
        raise SystemExit("unexpected V13 eval-state structure")
    source = source.replace(build_marker, build_marker + "    _build_nnue_cost_state_into(board, state)\n", 1)
    source = source.replace(advance_marker, advance_marker + "    _advance_nnue_cost_state_into(board, side, move, parent, child)\n", 1)
    if "correction = correction // 6" not in source:
        raise SystemExit("qualified V13 residual baseline missing")
