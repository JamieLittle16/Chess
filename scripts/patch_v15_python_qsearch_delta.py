...
new=f'''    for index in range(count):
        _pick_next_scored_move(moves, score_stack[ply], index, count)
        move = int(moves[index])
        # Delta pruning is useful only once another tactical line has raised alpha above stand-pat.
        # Keep promotions and en-passant unconditionally. For an ordinary capture, captured material
        # plus a generous positional margin must still be unable to reach alpha before it qualifies.
