...
if len(sys.argv)!=5:
    raise SystemExit('usage: patch_v15_python_razoring.py SEARCH.py MAX_DEPTH MARGIN1 MARGIN2')
...
insert=f'''    checked = _state_in_check(board, side, eval_stack[ply])
    # V15 verified razoring experiment. On shallow scout nodes whose cheap V13 static score is well
    # below alpha, use qsearch as a tactical verifier before paying for the full quiet tree. A node
    # is discarded only when qsearch also fails to raise alpha. PV/check/mate-score regions are kept.
    if (
        depth <= {max_depth}
        and beta == alpha + 1
        and not checked
